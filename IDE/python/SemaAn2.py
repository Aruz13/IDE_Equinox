...
#                raise ValueError(f"Type mismatch at line {node.line_no}")
    ...
    def print_symbol_table(self):
        ...
        f.write("--------------------------------------------------------------------------------\n")
        print("Nombre          | Tipo       | Valor             | # Registro (loc) | Líneas")
        print("--------------------------------------------------------------------------------")
        for name, symbol_info in self.symbols.items():
            for value, datatype, location in symbol_info["values"]:
                ...
                print(f"{name.ljust(15)} |  {datatype.ljust(9)} | {str(value).ljust(17)} | {str(location).ljust(16)} | {lines}")
                f.write(line_str)
...
def semantic_analysis(node, symbol_table):
    ...
    if datatype:
        ...
        for id_node in id_list.children:
            ...
            #print("STatus "+ id_node.value + str(status) )
            if( status == -1):
                id_node.val = "Error"
    ...
    if node.value == "Asignación":
        ...
        if not symbol_info:
            ...
            #print(node.children[1].type, " - " ,node.children[0].type, (node.children[1].type == "float" and node.children[0].type == "int"))
            if(node.children[1].type != node.children[0].type and (node.children[1].type == "float" and node.children[0].type == "int")):
            ...
            #TODO: SOLO SE ASIGNA Y SE GUARDA EN MEMORIA SI ES CORRECTO EL TIPADO, DLC, NO SE ACTUALIZA VARIABLE
               ...
               # node.children[0].val = var_value
                symbol_table.insert(var_name, node.val, var_type, None, node.line_no)
...
print("=-=> Arbol Anotaciones <=-=")
f = open("ArbolAnotaciones2.txt", "w", encoding="utf-8")
print_ast(ast, annotations= True)
f.close()
#Imprimir tabla de simboloas a txt
print("=-=> Tabla Simbolos <=-=")
f = open("TablaSimbolos2.txt", "w", encoding="utf-8")
symbol_table.print_symbol_table()
f.close()
#Imprimir errores semantico a txt
#print("=-=> Errores Semantica <=-=")
f = open("ErroresSemantico2.txt", "w", encoding="utf-8")
if errores_semantico:
